...
for page in tqdm(range(40), desc='Парсинг страниц...'):
    ...
    for mask in tag:
        ...
        sal = mask.find('span', attrs={'class': 'bloko-header-section-2'})
        if sal is not None:
            salary = mask.find('span', attrs={'class': 'bloko-header-section-2'}).text.replace('\u202f', '')
        ...
        if tag1 is not None:
            if tag:
                text = tag1.text
                match = re.search(r'\bDjango\b. \bFlask\b|\bFlask\b. \bDjango\b', text)
                if match:
